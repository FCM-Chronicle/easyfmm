...
class RealMatchEngine {
    ...
    generatePassEvent(isUser, minute) {
        ...
        else if (this.ballHolder.position === 'MF') targetPositions = ['FW', 'MF', 'DF'];
        else if (this.ballHolder.position === 'FW') targetPositions = ['FW', 'MF'];
    ...
    resolveGoalOutcome(outcome, isUserAttacking, minute, shootingQuality, existingCardElement = null, shooterName = null) {
        ...
        if (outcome === 'goal') {
            // ⚽ 골 성공
            // [수정] createGoalEvent에 실제 슈터 정보 전달 (함수 수정 필요 또는 여기서 덮어쓰기)
            // 기존 createGoalEvent는 랜덤으로 슈터를 정하므로, 여기서 생성된 이벤트를 수정합니다.
            event = createGoalEvent(this.matchData, isUserAttacking);
            
            // 만약 실제 슈터가 있다면 이벤트의 scorer를 수정 (단, createGoalEvent 내부 로직이 복잡하므로 
            // 여기서는 텍스트 일관성을 위해 createGoalEvent가 반환한 scorer를 그대로 두거나,
            // createGoalEvent를 수정해야 함. 간단히 여기서는 shooterName이 있으면 설명에 반영)
...
function displayEvent(event, matchData) {
    ...
    eventList.innerHTML = ''; // [수정] 이전 이벤트 지우고 최신 이벤트만 표시 (단순화)
