    ...
    initializePlayerGrowth() {
        ...
        teamPlayers.forEach(player => {
            if (player.age <= 25 && !this.growthData.has(player.name)) {
                ...
                const growthMonths = growthPotential <= 10 ? Math.max(5, growthPotential) : 14;
...
window.playerGrowthSystem = playerGrowthSystem;            "오현규": 94,
            "김민수": 98,
            "배준호": 93,
            "프란치스코 카마르다": 99,
            "제라르 마르틴": 90,
    ...
    grantPotentialToPlayer(player) {
        if (player.age <= 25 && !this.growthData.has(player.name)) {
            ...
            const monthlyGrowth = growthPotential / 14; // 14개월로 나눔
            ...
            console.log(`🌟 유망주 콜업: ${player.name}에게 성장 가능성 ${growthPotential} 부여 완료 (콜업 보너스 +${callUpBonus})`);
            return true;
        }
        return false;
    }
