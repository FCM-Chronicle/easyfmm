...
const SKILLS = {
    ...
    // 엘라스티코: 안으로 살짝 밀었다가 바깥으로 튕기듯 빠져나감
    ELASTICO: {
        name: '엘라스티코',
        minFrames: 16,
        buildPath(dir) {
            return [
                { t: 0.00, ox:  0,          oy:  0   },
                { t: 0.30, ox:  2.0 * dir,  oy:  0   },  // 안쪽 페이크
                { t: 0.55, ox: -0.5 * dir,  oy:  0   },  // 중심으로 순간 복귀
                { t: 0.80, ox: -3.5 * dir,  oy: -1.0 },  // 바깥으로 튕겨나감
                { t: 1.00, ox:  0,          oy:  0   },
            ];
        }
    },

    // 룰렛: 공 주위를 반원형으로 감아돌며 방향 전환 (지단 룰렛)
    ROULETTE: {
        name: '룰렛',
        minFrames: 24,
        buildPath(dir) {
            ...
            return [
                { t: 0.00, ox:  0,          oy:  0        },
                { t: 0.25, ox:  r * dir,    oy:  r * 0.5  },
                { t: 0.50, ox:  r * dir,    oy:  r * 1.2  },
                { t: 0.75, ox:  0,          oy:  r * 1.5  },
                { t: 1.00, ox:  0,          oy:  0        },
            ];
        }
    },
...
const SkillMoveManager = {
    ...
    */
    ...
    // 예: skillMoveManager.triggerManual('ROULETTE')
