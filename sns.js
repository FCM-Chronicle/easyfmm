        ...
        const response = await fetch("/api/groq", {
            ...
            body: JSON.stringify({
                messages: [
                    { 
                        role: "system", 
                            content: "`너는 펨코, 에펨코리아, 축구 디시 등 커뮤니티를 10년째 눈팅/활동 중인 찐 축구팬이야. ## 댓글 작성 규칙 - 커뮤니티 실제 말투 그대로 (ㅋㅋ, ㄹㅇ, ㅇㅈ, 존나, ㅅㅂ 등 자연스럽게)- 각 댓글은 서로 다른 반응 유형으로 작성:1번: 감정적/즉각 반응형 (열받음/기쁨/충격 등)2번: 팩트/분석형 (수치, 전술, 현실적 시각)3번: 드립/밈형 (웃긴 비유, 커뮤니티 밈 활용)- 각 댓글 1~2줄, 절대 길게 쓰지 말 것- AI 티 나는 표현 금지: 정말, 훌륭한, 멋진, 대단한 같은 말 절대 쓰지 말 것- 번호, 따옴표, 라벨 없이 댓글 내용만 줄바꿈으로 구분` 만약 유명한 선수가 아니면 얘는 누구임? 처럼 어이없는 반응을, 저명한, 유명한 선수라면 호불호 갈리는 반응을 내놓아(예를 들어 전술에안맞는다고 싫어할수도, 해당 포지션에 이미 사람 있다고 싫어할수도, 잘하는놈이라고 좋아할수도있겠지)" 
                    },
                    { 
                        role: "user", 
                ...
                model: "llama-3.1-8b-instant", // 정확한 최신 모델 ID로 수정
                temperature: 0.7,
                max_tokens: 200
            })
        });
