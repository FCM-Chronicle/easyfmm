            ...
            if (isLooseBall) {
                ...
                if (this.ball.state === BallState.IN_FLIGHT && p === this.ball.intendedReceiver) {
                    ...
                    if (isLastPasser && !isRearDefender) {
                        ...
                        // ─────────────────────────────────────────────────────────
                        // 오프사이드 라인 계산 (상대 필드 플레이어 두 번째 기준)
                        // ─────────────────────────────────────────────────────────
                        ...
                        // ─────────────────────────────────────────────────────────
                        // [핵심 수정] FW는 항상 오프사이드 라인 안쪽에서 대기
                        // burstTimer는 "침투 준비 상태"를 의미하며,
                        // 실제로 라인을 넘는 건 intendedReceiver로 지정됐을 때만
                        //
                        // burstTimer > 0: 라인 바로 앞(안쪽 2칸)에서 대기하며 타이밍 준비
                        // intendedReceiver: 패스가 실제로 나왔을 때만 라인 뒤로 달리기
                        // ─────────────────────────────────────────────────────────

                        if (p.burstTimer > 0) p.burstTimer--;

                        // 기본 대기 위치: 공 기준 전방 + 오프사이드 라인 안쪽 유지
                        let ballPushX = this.ball.x + (forwardDir * 20);
                        targetX = isHome
                            ? Math.min(offsideLimitX - 2, ballPushX)   // 라인 2칸 안쪽
                            : Math.max(offsideLimitX + 2, ballPushX);

                        // 중앙 FW 최소 전진 보장 (수비 라인이 낮아도 이 이상은 유지)
                        if (isCentralFW) {
                            const fwAbsLimit = isHome ? 68 : 32;
                            targetX = isHome
                                ? Math.max(targetX, fwAbsLimit)
                                : Math.min(targetX, fwAbsLimit);
                        } else {
                            const fwMinX = isHome ? 72 : 28;
                            targetX = isHome ? Math.max(targetX, fwMinX) : Math.min(targetX, fwMinX);
                        }

                        // burstTimer 중: 라인 바로 코앞(안쪽 1칸)까지 바짝 붙어 대기
                        // → 패스 나오는 순간 온사이드로 침투 가능
                        if (p.burstTimer > 0) {
                            const waitX = isHome ? offsideLimitX - 1 : offsideLimitX + 1;
                            targetX = isHome ? Math.min(targetX, waitX) : Math.max(targetX, waitX);
                            moveSpeed = 0.4 * speedFactor;
                        }

                        // 침투 타이밍: burstTimer 새로 발동 조건
                        if (p.burstTimer === 0) {
                        ...
                        // 공이 많이 뒤에 있을 때: 내려와서 패스 받기 (중앙 FW만)
                        ...
                        if (isOffside) {
                            targetX = isHome ? offsideLimitX - 2 : offsideLimitX + 2;
                            moveSpeed = 0.6 * speedFactor;
                        }
